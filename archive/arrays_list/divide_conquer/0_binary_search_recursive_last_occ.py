...
def binary_search(arr, low, high, x):
	...
	if high >= low:
		...
		print("Searching for array with indexes " + str(tab_str_low) + str(low) + tab_str_diff + str(
			high) + " where mid = " + str(mid) + " and element at mid is " + str(arr[mid]))

		# If element is present at the middle itself
		if arr[mid] == x and (mid == (len(arr) - 1) or x < arr[mid + 1]):  # Changed
			return mid
...
# Test array
arr = [2, 2, 3, 3, 4, 4, 4, 10, 40, 50, 60, 70, 80, 81, 82, 83, 84, 85, 86, 90, 91, 91, 92]
x = 2
