...
for inner_loop_index in range(len(array_arr)-1):
    #Check if the current element in bigger than the next element. If it is, push to the end
    if array_arr[inner_loop_index] > array_arr[inner_loop_index+1]:
        #swap elements
        array_arr[inner_loop_index], array_arr[inner_loop_index + 1] = array_arr[inner_loop_index+1], array_arr[inner_loop_index]
...
for outer_loop_index in range(len(array_arr)-1):
    for inner_loop_index in range(len(array_arr)-1-outer_loop_index):
        #Check if the current element in bigger than the next element. If it is, push to the end
        if array_arr[inner_loop_index] > array_arr[inner_loop_index+1]:
            #swap elements
            array_arr[inner_loop_index], array_arr[inner_loop_index + 1] = array_arr[inner_loop_index+1], array_arr[inner_loop_index]
